...
const TestimonialCard = ({ heading, body, name, role, initials, avatar, url, isActive }) => (
  ...
  return (
    <section className="wos-wrapper">
        ...
        {slides.map((t) => {
          ...
          return (
            <div
              ...
              className="wos-slide"
              data-pos={t.pos}
              onClick={() => t.pos !== 0 && setActiveIndex(mod(testimonials.indexOf(t), testimonials.length))}
            >
      ...
      <div className="wos-controls">
        <button className="wos-ctrl-btn" onClick={() => go(-1)}>‹</button>
        <button className="wos-ctrl-btn" onClick={() => go(1)}>›</button>
      </div>
    </section>
  );
